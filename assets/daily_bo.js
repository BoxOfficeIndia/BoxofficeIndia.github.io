...
async function getData()
{
    ...
    try
    {
        var response = await fetch(query_url, {
            method: 'GET', cache:cache_meth,//headers: headers,//body: JSON.stringify(json_payload),
        })
    }
    catch(err)
    {
    ...
    else if (end_point=='tsap')
    {
        ...
        modal_header_list = "Day No.,TS-AP Tracked gross <br>(in crore),Change"
        modal_data_columns = "Day count,Telugu States Gross,Change"
    }
    else if (end_point=='tamilnadu')
    {
        ...
        gr_cols = [4,6] //gross in cr/crore
        modal_header_list = "Day No.,TamilNadu Tracked gross <br>(in crore),Change"
        modal_data_columns = "Day count,TamilNadu Gross,Change"
    }
    else if (end_point=='karnataka')
    {
        ...
        gr_cols = [4,6] //gross in cr/crore
        modal_header_list = "Day No.,Karnataka Tracked gross <br>(in crore),Change"
        modal_data_columns = "Day count,Karnataka Gross,Change"
    }
    else if (end_point=='kerala')
    {
        ...
        gr_cols = [4,6] //gross in cr/crore
        modal_header_list = "Day No.,Kerala Tracked gross <br>(in crore),Change"
        modal_data_columns = "Day count,Kerala Gross,Change"
    }
    else if (end_point=='roi')
    {
        ...
        gr_cols = [4,6] //gross in cr/crore
        modal_header_list = "Day No.,ROI Tracked gross,Hindi Language Tracked gross <br>(in crore),Change"  //Hindi Gross Change % //North India
        modal_data_columns = "Day count,ROI Gross,Hindi Gross,Change"
    }
    else if (end_point=='multiplex')
    {
        header_list = ['Rank', 'Movie', 'Day', 'PIC Tracked gross', 'Tracked Shows', 'PVR Gross', 'INOX Gross', 'Cinepolis Gross', 'Top Cities', 'Statewise', 'Aggreagate ShowDown']
        data_columns = ['Rank', 'Movie', 'Day count', 'PIC Gross', 'PIC Shows', 'PIC json']
        gr_cols = [3,5,6,7]
        table_check = false
        table_html = create_multiplex_table(data, header_list, data_columns, gr_cols)
    }
    else
    {
    ...
    if (table_check) { table_html = create_table_withview(data, header_list, data_columns, text_sort_ids, init_table_html,
...
//function - multiplex page table
function create_multiplex_table(data, header_list, data_columns, gr_cols)
{
  table_html = init_table_html
  len = data.length;
  header_html = '<tr>'
  ...
  for(var i=0;i<len;i++){
    ...
    rank = j_data.Rank
    movie_name = j_data.Movie;
    day_count = j_data['Day count']
    pic_gross = j_data['PIC Gross']
    pic_shows = j_data['PIC Shows']
    pic_json = JSON.parse(j_data['PIC json'])
    pvr_gross = pic_json['PVR']
    cinepolis_gross = pic_json['Cinepolis']
    inox_gross = pic_json['INOX']
    td_html = ''
    row_data = [rank, movie_name, day_count, pic_gross, pic_shows, pvr_gross, cinepolis_gross, inox_gross]
